...
The weighted sum just described uses a single global set of layer weights, shared across every recording. A further refinement of this axis, evaluated in the unified configuration-driven reimplementation of the pipeline used to consolidate these techniques (\texttt{res\_model}), makes the combination adaptive: rather than one fixed weighting, an adaptive \emph{layer mixer} forms the softmax over layer logits conditionally, at one of three granularities. A \emph{global} mixer recovers the static weighted sum above; a \emph{sample} mixer predicts a separate weighting for each utterance, so that different recordings can emphasise different layers; and a \emph{token} mixer goes finer still, mixing the layers independently at every position. A temperature $\tau = 0.1$ sharpens the softmax so that the mixer commits to a few informative layers rather than spreading mass uniformly, and the mixer parameters are trained at roughly ten times the learning rate of the rest of the head. The per-utterance sample mixer is the variant carried into the recommended configuration: it learns a non-uniform, input-dependent weighting---placing more mass on the upper transformer layers---while remaining lightweight, and it generalises the standard fixed layer weighting into an input-conditioned one.
