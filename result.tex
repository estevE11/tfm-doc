...
\vspace{2pt}
{\footnotesize Text is the strongest single modality on every task; acoustic features alone are competitive, especially on amyloid. The best combination is task-dependent, and on the noisy PPA cohort adding the weak audio stream on top of text and features does not help under the pre-fusion token integration used here, though the features' benefit on PPA depends on where they enter (Table~\ref{tab:res_af_integration}). ADReSSo also reports test accuracy for the bimodal cells (Audio+Text 76.48\%). AF's controlled effect is in Table~\ref{tab:res_acoustic}.}
\end{table}
...
Where the features enter is itself a design choice, and under the fixed self-attention fusion of the modality grid it matters more than their mere presence. Holding the audio--text input fixed and varying only the integration point (Table~\ref{tab:res_af_integration}), amyloid is almost indifferent, within one point across the three options, but PPA is not: post-fusion concatenation reaches 65.13\% while the pre-fusion token used in the modality grid falls to 56.48\%, an 8.6-point swing from placement alone, and the post-fusion runs are also the steadier across seeds. ADReSSo is best with no features at all. This qualifies rather than contradicts the cross-attention ablation above: the benefit of the acoustic features is contingent on the task, on the fusion, and on where they join, and under the recommended cross-attention pipeline post-pooling concatenation slightly helps amyloid while slightly hurting PPA. With two seeds these PPA differences are read as indicative rather than settled.

\begin{table}[htbp]
\centering
\caption[Acoustic-feature integration point]{Acoustic-feature integration point (\texttt{res\_model}, fixed multi-head self-attention fusion, audio--text input, final Wav2Vec~2.0 layer, no alignment). \emph{None} = no AF; \emph{pre-fusion} = AF as an extra token inside the attention; \emph{post-fusion} = AF concatenated to the pooled representation after fusion. Mean CV validation accuracy (\%) over two seeds (43 and 3407).}
\label{tab:res_af_integration}
\small
\begin{tabular}{lccc}
\toprule
 & \multicolumn{2}{c}{\textbf{SPIN}} & \textbf{ADReSSo} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-4}
AF integration & \textbf{Amyloid} & \textbf{PPA} & AD \\
\midrule
None                 & 79.32 & 60.69 & \textbf{84.01} \\
Pre-fusion (token)   & \textbf{80.14} & 56.48 & 81.89 \\
Post-fusion (concat) & 79.22 & \textbf{65.13} & 73.17 \\
\bottomrule
\end{tabular}

\vspace{2pt}
{\footnotesize Where the features enter matters more than whether they are present: on PPA, post-fusion concatenation (65.13\%) beats the pre-fusion token (56.48\%) by 8.6~pp, while amyloid is indifferent and ADReSSo prefers no features. Task- and integration-dependent, not universal.}
\end{table}
...
\caption[Amyloid accuracy versus augmentation share]{Amyloid validation accuracy as the training augmentation share $p_{\mathrm{aug}}$ increases (\texttt{res\_model}, bidirectional cross-attention, character token-to-token alignment, final layer, with handcrafted features). Accuracy stays near the clean baseline (dashed) under light mixing and drops sharply once augmentation dominates; the 30\% share is the best setting and only matches, never beats, the clean baseline. Mean over seeds 43 and 3407, except the clean reference (seed 43).}
\label{fig:res_aug}
\end{figure}
