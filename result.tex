...
\caption{Validation accuracy of the fusion families on the three tasks (values from Table~\ref{tab:res_fusion}). The cross-attention family leads on every task, while Mamba's drop to chance on PPA stands out as the clearest outlier.}
...
\centering
\fbox{\parbox{0.9\linewidth}{\vspace{4cm}\centering\small\textit{[Figure to be added]}\vspace{4cm}}}
\caption{Validation accuracy against per-sample inference time for the fusion families (Step~B, ADReSSo). Cross-attention pairs high accuracy with low cost in the precomputed-embedding pipeline; absolute timings depend on whether the encoders are cached or run in the loop.}
...
\centering
\fbox{\parbox{0.9\linewidth}{\vspace{4cm}\centering\small\textit{[Figure to be added]}\vspace{4cm}}}
\caption{Learned per-layer weights of the per-utterance sample mixer on ADReSSo ($\tau = 0.1$, twelve Wav2Vec~2.0 layers). Weight concentrates on the uppermost layer with secondary mass on the lowest, and the per-utterance spread is shown as a shaded band around the mean.}
