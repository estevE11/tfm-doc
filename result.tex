...
This chapter reports the experimental programme laid out in Section~\ref{sec:methodology}. The presentation runs task by task first (Alzheimer's disease on ADReSSo, amyloid-$\beta$ status on SPIN, and PPA variant classification on the same SPIN subset), and then opens out into the per-axis ablations that explain where each task's accuracy comes from.

Two conventions hold throughout. ADReSSo is the only cohort with an official held-out partition, so it is the only task for which a \texttt{test-dist} accuracy is quoted alongside cross-validated validation accuracy; the two Spanish tasks are reported on validation folds only, and no artificial test split is ever substituted for the one they lack. Every accuracy is a mean over the five cross-validation folds and the two training seeds (43 and 3407), given as a point estimate. No significance testing was carried out, a limitation revisited in the Discussion.

Unless a table says otherwise, the numbers come from the unified \texttt{res\_model} implementation run under a single stable recipe (weighted Wav2Vec~2.0 layers, mean pooling, and no handcrafted features) so that one axis moves at a time. Where the older CogniAligned and SER grids supply something the unified runs cannot, namely a second independent stack for the fusion and pooling trends or the legacy alignment sweep on amyloid, those numbers are brought in explicitly and labelled as such.

\subsection{Alzheimer's Disease Classification (ADReSSo)}
\label{ssec:res_adresso}

ADReSSo is the anchor task. It is a balanced, age- and sex-matched English benchmark with a fixed test partition, which makes it the one place in the thesis where a model can be judged against an external, previously unseen split rather than on its own folds.

The recommended configuration pairs bidirectional cross-attention fusion with word-to-token alignment and a weighted stack of Wav2Vec~2.0 layers refined by the per-utterance sample mixer of Section~\ref{sssec:pooling}. Averaged over the five folds and both seeds it reaches \textbf{88.16\%} validation accuracy. Trained once on the full labelled set and evaluated on the official \texttt{test-dist} partition, the pipeline scores \textbf{83.10\%} test accuracy (Table~\ref{tab:res_best}), which sits among the stronger systems reported for the challenge and was obtained without any leaderboard-specific tuning.

One result deserves to be flagged early, because it returns in the ablations. On ADReSSo the Mamba fusion block is not merely competitive: it edges the validation grid (88.44\% in the legacy Step~B sweep) and supplies the 83.10\% test figure after full training. That is a genuine finding and it is reported as one. It does not, however, make Mamba the recommended default. On the Spanish PPA task the same block collapses to near chance (Section~\ref{ssec:res_ablation}), and its ADReSSo advantage is sensitive to the seed. The recommendation therefore keeps the more even-tempered bidirectional cross-attention, trading a little ADReSSo accuracy for behaviour that holds up across all three tasks.

\subsection{Amyloid-$\beta$ Status Prediction (SPIN)}
\label{ssec:res_amyloid}

The amyloid task asks the model to recover a fluid biomarker (amyloid positivity) from forty seconds of Spanish picture description, a label otherwise confirmed only by a lumbar puncture or a PET scan. There is no official test partition, so every figure is a five-fold cross-validation accuracy.

Under the unified stable recipe, gated cross-attention is the best plain fusion at 82.88\% (Table~\ref{tab:res_fusion}), and the task leans noticeably on the text stream (Table~\ref{tab:res_modality}). The headline number, though, comes from the configuration this thesis contributes. With character-proportional token-to-token alignment, a final-layer audio representation, bidirectional cross-attention, and the 55-dimensional handcrafted acoustic features switched on, a tuned run reaches \textbf{83.90\%} validation accuracy (Table~\ref{tab:res_best}).

The alignment effect is worth isolating here, because it is the methodological core of the thesis and amyloid is where it shows most clearly. In the legacy Tier~A grid (bidirectional cross-attention, handcrafted features, final-layer audio, everything else fixed), character-proportional token-to-token alignment reaches \textbf{85.72\%}, against \textbf{85.13\%} for the CogniAlign word-to-token baseline and the same 85.13\% for the uniform variant. The proportional scheme is the only one of the three to move the result, and it moves it the right way ($+0.59$~pp). The unified \texttt{res\_model} re-implementation reproduces the recipe at 83.90\% rather than 85.72\%; that gap reflects differences in the training stack, not a reversal of the trend, and the two figures should be read together as reproduction status rather than as competing claims.

\subsection{PPA Variant Classification (WAB)}
\label{ssec:res_ppa}

PPA variant classification is the hardest of the three tasks and the one whose numbers carry the most caveats. It is a three-class problem over a small, imbalanced cohort (35, 22, and 17 subjects for the logopenic, non-fluent, and semantic variants), so fold-level accuracy is noisy, and a handful of misassigned recordings shift the mean appreciably.

The best configuration uses bidirectional cross-attention, uniform token-to-token alignment, and a final-layer audio representation, reaching \textbf{74.95\%} validation accuracy (Table~\ref{tab:res_best}). Two patterns set PPA apart from the other tasks. It is the only task that prefers audio to text as a single modality (60.03\% against 53.57\%, Table~\ref{tab:res_modality}), which fits the motor-speech and fluency disturbances that mark the non-fluent variant. And it is unforgiving of the wrong fusion: the Mamba block, competitive elsewhere, falls to 52.57\% (barely above the majority class), while the cross-attention family stays in the low seventies (Table~\ref{tab:res_fusion}). Given the cohort size, these results are read as indicative rather than definitive.

\subsection{Ablation Studies}
\label{ssec:res_ablation}

The per-task numbers above are summaries of a larger grid. This section walks that grid one axis at a time, in the order the pipeline is built: which modalities to feed in, whether handcrafted features help, how to fuse the streams, how to pool the Wav2Vec layers, how to align audio to text, and which pretrained encoders to use. Each axis is varied with the others pinned to the stable recipe, so that a change in accuracy can be charged to the axis under test. For the two axes that a second, independent implementation also covers (fusion and Wav2Vec pooling), its agreement with the unified runs is noted.

\subsubsection{Modality}

Naive combination of the two streams is not enough. Concatenating or averaging the audio and text embeddings (Table~\ref{tab:res_modality}) never beats the better of the two unimodal branches, and on the Spanish tasks it sits well below it. The tasks also disagree about which modality carries the signal: amyloid is text-led (67.85\% text against 66.23\% audio), whereas PPA is audio-led (60.03\% against 53.57\%); ADReSSo, like amyloid, prefers text. The practical lesson is that extracting value from both modalities at once needs an attention-based fusion able to learn the correspondence between them. Simple pooling leaves accuracy on the table.
...
\begin{tabular}{lccc}
\toprule
Input configuration & \textbf{Amyloid} & \textbf{PPA} & ADReSSo \\
\midrule
...
\begin{tabular}{lccc}
\toprule
Acoustic features & \textbf{Amyloid} & \textbf{PPA} & ADReSSo \\
\midrule
...
Fusion is the axis with the widest spread (Table~\ref{tab:res_fusion}, shown graphically in Figure~\ref{fig:res_fusion_bar}), and the three groups behave exactly as their inductive biases would predict. Pooling and plain self-attention stay near chance on the Spanish tasks; self-attention over an undifferentiated audio--text bag has no way to find the cross-modal structure on its own. Positional information and multiple heads help, but the decisive jump comes from keeping the modalities apart and letting one attend to the other. The cross-attention family (plain, gated, and bidirectional) tops every task: between 81 and 83\% on amyloid, the low seventies on PPA, and the high eighties on ADReSSo. Mamba is the telling outlier. It is strong on English and amyloid (85.40\% on ADReSSo, 77.49\% on amyloid) yet collapses to 52.57\% on PPA, and that collapse is what rules it out as a general-purpose default despite its ADReSSo standing.
...
\legend{Amyloid, PPA, ADReSSo}
\end{axis}
\end{tikzpicture}%
}
\caption[Fusion-family accuracy across tasks]{Validation accuracy of the fusion families on the three tasks (values from Table~\ref{tab:res_fusion}). The cross-attention family leads on every task, while Mamba's drop to chance on PPA stands out as the clearest outlier.}
\label{fig:res_fusion_bar}
\end{figure}

This ranking is the main trend the second, independent SER stack was built to check, and it survives the change of implementation. In the legacy Step~B grid the cross-attention and Mamba families again lead within SER (bidirectional cross-attention reaches 85.21\% validation on ADReSSo, for instance), even though SER uses different encoders, a different ASR path, and a shorter training schedule. The absolute numbers diverge between the two stacks, which are not a controlled head-to-head, but the ordering of fusion families holds, and reproducing that ordering twice is the whole point of running it twice. The stacks also differ starkly in inference cost. Because CogniAligned reads precomputed token-aligned tensors while the SER timings include the encoders in the loop, per-sample inference differs by more than two orders of magnitude: sub-millisecond against a couple of hundred milliseconds (Figure~\ref{fig:res_compute}). That gap is a property of the embedding cache, not of the fusion mechanism, and it is what makes a multi-axis grid affordable in the first place.
...
\begin{axis}[
    ...
    xlabel={Per-sample validation inference time (ms)},
    ylabel={Validation accuracy (\%)},
    xmin=-0.1, xmax=2.0, ymin=73, ymax=90,
    ymajorgrids, grid style={gray!20},
    scatter/classes={a={mark=*,blue!60}},
]
\addplot[
    scatter, only marks, mark=*, mark size=2.4pt,
    point meta=explicit symbolic,
    nodes near coords, nodes near coords align={anchor=west},
    every node near coord/.append style={font=\scriptsize},
] coordinates {
    (1.06,75.70) [Self-attn]
    (0.05,75.66) [MH self-attn]
    (1.68,83.01) [Transf.+PE]
    (0.40,86.63) [Cross]
    (0.64,86.61) [Gated cross]
    (0.59,84.79) [Bi-cross]
    (0.96,88.44) [Mamba]
};
\end{axis}
...
\caption[Accuracy versus inference time]{Validation accuracy against per-sample inference time for the fusion families in the precomputed-embedding (CogniAligned) pipeline (Step~B, ADReSSo). Cross-attention pairs high accuracy with low cost; every family stays under two milliseconds because the encoder outputs are cached, whereas running the encoders in the loop raises per-sample cost by roughly two orders of magnitude.}
\label{fig:res_compute}
\end{figure}

\begin{table}[htbp]
\centering
\caption[Fusion-technique ablation per task]{Fusion-technique ablation per task (\texttt{res\_model}, weighted Wav2Vec~2.0 layers, no acoustic features). Mean CV validation accuracy (\%) over seeds. Fixed per-task alignment: ADReSSo tok$\rightarrow$tok (char); amyloid word$\rightarrow$token; PPA tok$\rightarrow$tok (uniform).}
\label{tab:res_fusion}
\small
\begin{tabular}{lccc}
\toprule
Fusion technique & \textbf{Amyloid} & \textbf{PPA} & ADReSSo \\
\midrule
...
\vspace{2pt}
{\footnotesize Cross-attention variants dominate on every task; Mamba is competitive on English and amyloid but collapses on PPA. Naive concat/mean/self-attention fusions stay near chance on SPIN.}
\end{table}

\subsubsection{Wav2Vec Layer Pooling and the Layer Mixer}

Two questions sit on the audio branch: whether to read the final Wav2Vec layer or a learnable mixture of all twelve, and, if a mixture, whether a single global set of weights is enough. The pooling table (Table~\ref{tab:res_wav2vec_pool}) gives a muted answer. A weighted layer stack helps ADReSSo slightly (86.64\% against 86.32\%), but on the Spanish tasks the final layer is as good or better, clearly so on PPA (74.95\% against 72.43\%). Depth, once a competent fusion is in place, buys little.
...
\begin{tabular}{lccc}
\toprule
Pooling & \textbf{Amyloid} & \textbf{PPA} & ADReSSo \\
\midrule
...
\begin{tabular}{llccc}
\toprule
Alignment & Source & \textbf{Amyloid} & \textbf{PPA} & ADReSSo \\
\midrule
...
\toprule
Variant & Text encoder & Audio encoder & \textbf{Amyloid} & \textbf{PPA} & ADReSSo \\
\midrule
...
\subsubsection{Unified-Model Confirmation}

Taken as a whole, the unified \texttt{res\_model} runs reproduce the main trends established on the two primitive stacks (the cross-attention family wins fusion, depth beyond the final layer adds little, and subword alignment helps narrowly and task-specifically), while adding the two studies the primitive stacks could not express cleanly: the adaptive layer mixer and the single-factor encoder swaps. Table~\ref{tab:res_best} collects the strongest bundle found for each task. No two of them share the same recipe, which is the thesis's central empirical message in miniature: the best choice along one axis depends on the task, and a single configuration that wins everywhere does not exist in this grid. What the evidence supports instead is a sensible default (bidirectional cross-attention, word-to-token alignment, a weighted audio stack with the sample mixer, and handcrafted features off) that stays close to each per-task best while remaining stable across folds, seeds, and tasks.
...
ADReSSo (ref.)          & bicross, word$\rightarrow$token, weighted Wv $+$ sample mixer & 88.16 & 83.10$^{b}$ \\
\bottomrule
\end{tabular}

\vspace{2pt}
{\footnotesize $^{a}$Best single run (seed 43, lr $1.5\times10^{-5}$, dropout 0.25) with the 55-d handcrafted acoustic features; the plain-recipe gated cross-attention reaches 82.88\%. $^{b}$Official ADReSSo test-dist under the full-train protocol (Mamba, word$\rightarrow$token, final layer), the recipe that topped the ADReSSo fusion grid.}
\end{table}
