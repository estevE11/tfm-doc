...
Two configurations of this model are relevant to the present work. The English \texttt{base-960h} model, trained on read English speech, is one; the multilingual XLSR-300M variant, which extends the same framework to over 300{,}000 hours of speech spanning 128 languages, is the other, and its cross-lingual pre-training makes it better suited to scenarios where labelled pathological speech is scarce in any individual language, such as the Spanish cohorts studied here. The thesis employs both as frozen feature extractors and compares them directly in a controlled encoder ablation, since the more powerful multilingual model is not guaranteed to win once the downstream task is fixed.
