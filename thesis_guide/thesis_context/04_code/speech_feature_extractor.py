...
class SpeechFeatureExtractor(nn.Module):
    ...
    def __init__(self, input_parameters):
        ...
        self.wav2vec_layer_mode = getattr(input_parameters, 'wav2vec_layer_mode', 'weighted')
        if self.wav2vec_layer_mode not in ('weighted', 'last'):
            raise ValueError(f"wav2vec_layer_mode must be 'weighted' or 'last', got {self.wav2vec_layer_mode!r}")
        self.init_feature_extractor()
        if self.wav2vec_layer_mode == 'weighted':
    ...
    def init_feature_extractor(self):
        ...
        if self.speech_feature_extractor_type == "WAV2VEC2_BASE":
            bundle = torchaudio.pipelines.WAV2VEC2_BASE
            self.num_layers = 12
        elif self.speech_feature_extractor_type == "WAVLM_BASE":
    ...
    def extract_features(self, waveform):
        ...
        if self.wav2vec_layer_mode == 'last':
            return features[-1]

        hidden_states = torch.stack(features, dim=1)
        return (hidden_states * self.layer_weights.view(-1, 1, 1)).sum(dim=1)
